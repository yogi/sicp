...
(define (augend s) 
  (if (null? (cdddr s))
      (caddr s)
      (make-terms-sum (cddr s))))

(define (multiplicand p)
    (if (null? (cdddr p))
      (caddr p)
      (make-terms-product (cddr p))))
...
(assert-equals 5 (augend '(+ 1 2 3)))
(assert-equals 6 (multiplicand '(+ 1 2 3)))
(assert-equals '(+ b (+ c d)) (augend '(+ a b c d)))
(assert-equals 7 (deriv '(+ x 3 4 (* 2 x 3)) 'x))
